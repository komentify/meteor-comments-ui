...
Meteor.methods({
  ...
  'comments/star': function (documentId, starsCount, anonUserData) {
    ...
    if (CommentsCollection.findOne({ _id: documentId, 'starRatings.userId': userId })) {
      CommentsCollection.update({ _id: documentId }, { $pull: { starRatings : { userId } } }, noOptOptions);
    }

    CommentsCollection.update({ _id: documentId }, { $push: { starRatings: { userId, rating: starsCount } } });
  },
