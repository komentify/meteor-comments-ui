...
Meteor.methods({
  'comments/add': function (referenceId, formdata, anonUserData) {
    check(referenceId, String);
    check(formdata.content, String);

    userService.verifyAnonUserData(anonUserData);
    const userId = this.userId || anonUserData._id;

    if (userId && formdata.content) {
      p = {
          referenceId,
          userId,
          createdAt: (new Date()),
          ...
          media: mediaService.getMediaFromContent(formdata.trimmedValue)
      };
      for (var attr in p) { formdata[attr] = p[attr]; }
      return (CommentsCollection.insert(formdata));
    }
  },
  ...
  'comments/reply/add': function (documentId, docScope, formdata, anonUserData) {
    check(documentId, String);
