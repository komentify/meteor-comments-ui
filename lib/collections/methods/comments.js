...
const getReferenceIdByDocumentId = documentId => CommentsCollection.findOne({ _id: documentId }, { fields: { referenceId: 1 } }).referenceId
...
Meteor.methods({
  ...
  'comments/reply/like': function (documentId, docScope, anonUserData) {
    ...
    if (!userId || !allowReplies(documentId) || Comments.config().rating !== 'likes') {
      return false
    }
