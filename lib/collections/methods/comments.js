...
import { validateCaptchaIfRequired } from '../../services/captcha-service'
...
const getReferenceIdByDocumentId = documentId => {
...
function hasCaptchaOnClient(anonUserData) {
  return Comments.config().sweetCaptcha && anonUserData && Meteor.isClient
}

// TODO: add unit tests + good user testing (bootstrap, ionic and so on)!
Meteor.methods({
  'comments/add': async function (referenceId, content, captchaData, anonUserData) {
    check(referenceId, String)
    check(content, String)
    check(captchaData, Object)
    ...
    await validateCaptchaIfRequired(config, captchaData)

    if (hasCaptchaOnClient(anonUserData)) {
      return false
    }
  ...
  'comments/reply/add': async function (documentId, docScope, content, captchaData, anonUserData) {
    check(documentId, String)
...
CommentsCollection.methods = {
  add: (referenceId, content, captchaData, cb) => callWithAnonUserData.bind({ cb }, 'comments/add', referenceId, content, captchaData)(),
  reply: (documentId, docScope, content, captchaData, cb) => callWithAnonUserData.bind({ cb }, 'comments/reply/add', documentId, docScope, content, captchaData)(),
