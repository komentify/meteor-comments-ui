...
CommentsCollection.schemas.ReplySchema = new SimpleSchema(
  getCommonCommentSchema({
    replyId: {
      type: String
    }
  })
...
CommentsCollection.schemas.CommentSchema = new SimpleSchema(
  getCommonCommentSchema({
    referenceId: {
      type: String
    }
...
const getCount = (scope, field) =>
  scope[field] && scope[field].length ? scope[field].length : 0;

addCollectionMethods(CommentsCollection);
addReplyMethods(CommentsCollection);
...
CommentsCollection.helpers({
  ...
  getStarRating: function() {
    if (_.isArray(this.starRatings)) {
      const ownRating = _.find(
        this.starRatings,
        rating => rating.userId === Meteor.userId()
      );
