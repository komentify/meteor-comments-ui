...
const getUserIdForPublication = (referenceId, userId, anonUserData) => {
  try {
    userId = userService.verifyAnonUserData(anonUserData, referenceId)
  } catch(e) {}
...
Meteor.publishComposite('comments/reference', function (id, anonUserData, sorting, limit, skip = 0) {
  ...
  return getCompositeCommentCursor(Comments.getCollection().find(
    selector,
    {
      limit,
      skip,
