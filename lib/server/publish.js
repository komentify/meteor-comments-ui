const linkifyStr = Npm.require('linkifyjs/string');
...
Meteor.publishComposite('comments/reference', function (id, sorting, limit, skip = 0) {
  ...
  return {
    find: () => Comments._collection.find(
      { referenceId: id },
      { limit, skip, sort: Comments.getSortOption(sorting), transform: (doc) => { doc.enhancedContent = linkifyStr(doc.content); return doc; } }
    ),
    children: [{
      find(comment) {
