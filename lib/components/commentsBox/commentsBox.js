...
const addComment = function (event, type, selector, callback) {
  ...
  if ("submit" === event.type) {
    ...
    executeUserAction(event, type, function (anonymousData) {
      ...
      var formdata = Comments.config().beforeInsert(event);
      formdata.trimmedValue = textarea.val().trim();

      callback(textarea, formdata, anonymousData);
    });
  }
};
...
Template.commentsBox.events({
  ...
  'submit .comment-form' : function (e) {
    ...
    addComment(e, 'add comments', '.create-comment', function (textarea, formdata, anonData) {
      function addComment() {
        Comments.add(eventScope.id, formdata);
        textarea.val('');
      }

      if (formdata.trimmedValue) {
        if (anonData) {
  ...
  'submit .reply-form' : function (e) {
    ...
    addComment(e, 'add replies', '.create-reply', function (textarea, formdata, anonData) {
      function addReply() {
        const id = eventScope._id || eventScope.documentId;

        Comments.reply(id, eventScope, formdata);
