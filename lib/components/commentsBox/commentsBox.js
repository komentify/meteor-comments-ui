  ...
  if ("submit" === e.type) {
    ...
    if (userService.isAnonymous()) {
      addCommentCallback({
        ...
        mail: container.find('.anon-email').val()
      });
    } else {
...
Template.commentsBox.onCreated(function () {
  ...
  this.autorun(function () {
    const userId = Meteor.userId(),
      anonymousAllowed = Comments.config().anonymous;

    if (anonymousAllowed || userId) {
  ...
  'submit .comment-form' : function (e) {
    ...
    addComment(e, 'add comments', '.create-comment', function (textarea, trimmedValue, anonData) {
      if (trimmedValue) {
        ...
        Comments.add(eventScope.id, trimmedValue, anonData);
        textarea.val('');
      }
    });
  },
