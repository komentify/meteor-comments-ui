Comments = (function () {
  ...
  collection.attachSchema(CommentsSchema);
  ...
  function transformReplies(scope, position) {
    ...
    return _.map(scope.replies, function (reply, index) {
      ...
      // TODO: loop through all the data
      ...
      if (reply.replies) {
        // recursive!
        reply.enhancedReplies = _.bind(transformReplies, null, _.extend(scope, { replies: reply.replies }), position)();
      }
  ...
  Meteor.methods({
    'comments/add': function (referenceId, content) {
      ...
      collection.insert(
          { referenceId: referenceId, content: content, userId: this.userId, createdAt: (new Date()), likes: [] }
      );
    },
    ...
    'comments/reply/add': function (documentId, docScope, content) {
      ...
      if (docScope._id) {
        ...
        modifyNestedReplies(doc.replies, docScope.position, function (replies, index) {
          replies[index].replies.push(reply);
        });
      }
    ...
    'comments/reply/remove': function (documentId, docScope) {
      ...
      collection.update({ _id: documentId, userId: this.userId }, { $set: { replies: doc.replies }  }, noOptOptions);
    },
